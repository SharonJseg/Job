...
export class JobDetailsComponent implements OnInit {
  ...
  private getJob(): Observable<IJob | undefined> {
    return this.activatedRoute.queryParams.pipe(map((param) => {
      console.log('params:', param);
      this._currentId = param.getUrl("id");
  ...
  next() {
    return this.router.navigate(["/job-details/" + ((this._currentId as number) + 1)]);
  }
}
